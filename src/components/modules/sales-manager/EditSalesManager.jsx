...
const EditSalesManager = () => {
    ...
    const [nid_img , setNIDImg] = useState('');
    ...
    const getSalesManagerData = ()=>{
        axios.get(`${Constants.BASE_URL}/sales-manager/${params.id}`).then(res=> {
            setInput(res.data.data);
            setPhoto(res.data.data.old_photo);
            getDistrict(res.data.data.division_id);
            getArea(res.data.data.district_id);
        })
    }
    const getDivision =()=>{
    ...
    const  salesManagerUpdateForm = async ()=>
    {
        ...
        formData.append('nid_photo',nid_img);
        formData.append('_method','PUT');

        await axios.post(`${Constants.BASE_URL}/sales-manager/${params.id}`,formData, {
    ...
    const  handleSalesManagerUpdate = async (e)=>{
        e.preventDefault();
        await salesManagerUpdateForm();
    }

    return (
        <>
            <Breadcrumb title={"Update Sales Manager"} />
            <div className="row">
                <div className="col-md-12">
                    {success && <Alert className="col-sm-3 offset-3"  variant="danger">{success}</Alert>}
                    <div className="card">
                        <div className="card-header">
                            <CardHeader title={'Update Sales Manager'} link={'/sales-manager'} button_text={'Sales Manager List'} icon={'fa-list'} />
                        </div>
                        <div className="card-body">
                            <form onSubmit={handleSalesManagerUpdate}>
                                            ...
                                            <div className="card-body">
                                                ...
                                                <label className={'w-100'}>
                                                    ...
                                                    <textarea className={errors.bio != undefined ? "form-control is-invalid" : "form-control"}
                                                              name={"bio"} onChange={handleInput} value={input.bio}   > </textarea>
