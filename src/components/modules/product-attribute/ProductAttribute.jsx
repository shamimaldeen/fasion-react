...
const ProductAttribute = () => {
    ...
    const handleCreateModal = (attribute = null)=>{
        if (attribute != null){
            setModalTitle("Update");
            setIsEditMode(true);
            setInput( {name:attribute.name,status: attribute.status,id:attribute.id });
        }else{
            setInput( {status: 1});
            setModalTitle("Add")
            setIsEditMode(false);
    ...
    const  productAttributeForm = async ()=>
    {
        const  formData = new FormData();
        formData.append('name',input.name);
        formData.append('status',input.status);
        await axios.post(`${Constants.BASE_URL}/attribute`,formData, {
    ...
    const  handleAttributeCreate = async (e)=>{
        e.preventDefault();
        await productAttributeForm();
    }

    const  handleAttributeUpdate = async (e)=>{
        e.preventDefault();
        await handleAttributeUpdateForm();
    }


    const  handleAttributeUpdateForm = async ()=>
    {
        ...
        await axios.post(`${Constants.BASE_URL}/attribute/${input.id}`,formData, {
            headers: {'Content-Type': 'multipart/form-data' },
        }).then(response=> {
            setIsloading(false);
            Swal.fire({
                                            ...
                                            <table className={'my-table table table-hover table-striped table-bordered'}>
                                                ...
                                                </thead>
                                                {
                                                    isLoading ? <Loader />
                                                        ...
                                                        <tbody>
                                                        {
                                                            Object.keys(attributes).length > 0 ?   attributes && attributes.map((attribute,index) => (
                                                                <tr key={index}>
                                                                    <td>{ index +1 }</td>
                                                                    <td>{attribute.name}</td>
                                                                    <td>{attribute.status == 1 ? <strong style={{ color:"green" }}>Active</strong> : <strong style={{ color:"red" }}>Inactive </strong> }</td>
                                                                    <td>

                                                                       <button className={'btn btn-sm btn-warning my-1 mx-1'} onClick={() => handleCreateModal(attribute)} ><i className="fa-solid fa-pen-to-square"></i></button>
                                                                        <button className={'btn btn-sm btn-danger my-1'} onClick={() => handleAttributeDelete(attribute.id)} ><i className="fa-solid fa-trash"></i></button>
                                                                    </td>
                                                                </tr>
                                                            )) : <NoDataFound /> }
                                                        </tbody>
              ...
              <Modal
                ...
                <Modal.Header closeButton>
                    <Modal.Title id="category-details-modal">
                        <h4> {modalTitle} Product Attribute</h4>
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                <form onSubmit={ isEditMode ? handleAttributeUpdate :handleAttributeCreate}>
                    <div className="col-md-12">
